    ...
    <div className="text-text">
      ...
      <div className="border-1 border-border rounded-xl p-4 mb-4">
          <table className='text-text w-full'>
              ...
              {formattedCreditors.map((creditor) => !debts.find(
                (debt) => debt.creditor.userId === creditor.userId)?.settled && (
                <tr className="w-full border-b border-border" key={creditor.userId}>
                  {/* <CreditDetails creditor={creditor} debts={debts} /> */}
                  <h1>ROWS</h1>
                </tr>
              ))}
            </tbody>
      ...
      <div className="border-1 border-border rounded-xl p-4">
          <table className='text-text w-full'>
              ...
              <tbody className='text-text [&_tr:last-child]:border-0'>
              {formattedDebtors.map((debtor) => (!credits.find((credit) => credit.debtor.userId === debtor.userId)?.settled && (
                <tr className="w-full border-b border-border" key={debtor.userId}>
                  <DebtDetails debtor={debtor} credits={credits} />
                </tr>
              )))}
            </tbody>
          </table>
