...
export const Graph = ({expenses}) => {
  ...
  useEffect(() => {
    ...
    setDates(lastWeekExpenses.map(expense => moment(expense.createdAt).format('dddd')));
    setWeeklyAmounts(lastWeekExpenses.map(expense => expense.amount));
    setMonths(lastMonthExpenses.map(expense => moment(expense.createdAt).format('MMMM')));
  ...
  return (
    <div className='col-span-2 grid grid-cols-2 gap-8 text-text flex-1'>
