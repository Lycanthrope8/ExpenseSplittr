...
const HomeTaskDetails = ({ task, onDelete, onEdit }) => {
    ...
    <>
      <form id="editingForm" className='flex w-full' onSubmit={handleEdit}>
        <div className='flex flex-col flex-1'>
          <input className='text-3xl border-b-1 mb-2 bg-transparent outline-none' placeholder={task.title} onChange={(e) => setNewTitle(e.target.value)} />
          <input className="text-lg mb-4 bg-transparent outline-none" placeholder={task.description} onChange={(e) => setNewDescription(e.target.value)} />
          <input type="date" className='text-lg mb-4 text-text bg-transparent outline-none' placeholder={task.deadline} onChange={(e) => setNewDeadline(e.target.value)} />
  ...
  return (
    <div className="task-details flex justify-between text-text p-4 mb-4 rounded-2xl">
